...
export default function FixedControls({ refreshVessels, setInfo, fetching }) {
  ...
  return (
    <div className="fixed top-4 right-2 flex flex-col z-50 items-center">
      <button 
        onClick={() => { refreshVessels(); setSpinning('animate-spin-once') }} 
        onAnimationEnd={() => { setSpinning('')}}
      ...
      <button onClick={setInfo} className="bg-green-700 active:bg-green-900 rounded-full h-10 w-10 p-2 mt-4 shadow-lg select-none">
